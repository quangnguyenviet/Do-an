...
| -------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------- |
| **Parent Portal**          | Landing page, Form khảo sát nhu cầu trực quan, danh sách gia sư kèm Matching Score, đăng ký học thử tức thì (OTP SMS), FAQ                                                                                                                                                                          | FR-1, FR-2, FR-3, FR-4           |
...
| 2.2           | Module Parent/Match Request (FR-1→FR-4)                                                                                        | API khảo sát, matching score, đăng ký học thử    |
...
| Tuần                   | Giai đoạn                 | Module chính               | Công việc cụ thể                                                                                                                                                           |
| ----------------------- | --------------------------- | --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1                       | Khởi tạo                  | Hạ tầng & Thiết kế      | Thiết kế ERD, setup Spring Boot skeleton, Docker Compose; Dựng prototype ReactJS 4 cổng; Thiết kế LangGraph flow, setup AI service skeleton                            |
| 2                       | Nền tảng                  | Auth & Layout chung         | Auth JWT + RBAC theo Enrollment; Layout chung, routing, component thư viện dùng chung; Xây prompt template Curriculum Agent                                                |
| 3                       | Parent Portal               | Module Phụ huynh           | API + UI FR-1→FR-4 (khảo sát, matching score, đăng ký học thử, FAQ); Hoàn thiện Curriculum Agent (Option A)                                                                |
| 4                       | Tutor — Curriculum         | Module Gia sư              | API + UI nhận lớp + QR proof; Cấu hình lịch học thử & chốt lịch cố định (FR-29); UI Curriculum Dual-Mode; Parser Import Option B (Curriculum)                            |
| 5                       | Tutor — Assistant AI       | Module Gia sư + AI Service | UI Tutor Assistant; API FR-8, FR-9 (sinh bài, duyệt & giao bài); Tutor Assistant Agent (sinh câu hỏi/đáp án/giải thích); Parser Import Option B (bài tập)          |
| 6                       | Tutor — vận hành         | Module Gia sư              | API + UI Rate Card, Private Notes, Lịch dạy, Đổi lịch/Báo nghỉ; Upload tài liệu/S3; Content Moderation + Privacy filter                                               |
| 7                       | Student Portal              | Module Học sinh            | API + UI FR-11→FR-13 (làm bài, chấm điểm, giải thích AI 2 chế độ); FR-15 (báo cáo tiến bộ); FR-22 (xem tài liệu/video)                                        |
| 8                       | Admin Dashboard             | Module Admin                | API + UI FR-16, FR-23 (Match Request, Match Offer, duyệt phí QR, mở khóa liên hệ); FR-19, FR-20, FR-24 (RBAC audit, báo cáo vận hành); Tối ưu độ trễ AI Service |
| 9                       | Tích hợp toàn hệ thống | Tích hợp                  | Tích hợp BE↔AI Service qua API Gateway; Nối toàn bộ FE với API thật; Rà soát RBAC; Kiểm thử tải AI Service, benchmark <5s; Test luồng UJ-1→UJ-4                 |
| 10                      | Kiểm thử & Triển khai    | Kiểm thử & Triển khai    | Fix bug; Unit test + Integration test; UAT; Deploy staging; Viết API doc, tài liệu kỹ thuật, hướng dẫn sử dụng                                                       |
| 10.5 (2–3 ngày cuối) | Bàn giao                   | Bàn giao                   | Đóng gói Docker production; Chuẩn bị & sắn sàng demo                                                                                                                   |
